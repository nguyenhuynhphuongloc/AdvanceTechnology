...
export function validateEnvironment(config: Record<string, unknown>) {
  ...
  if (missingDbKeys.length > 0) {
    throw new Error(`Missing required inventory-service config: ${missingDbKeys.join(', ')}`);
  }

  if (isEnabled(config, 'REDIS_ENABLED')) {
    const redisUrl = config.REDIS_URL;
    if (!isTest && (typeof redisUrl !== 'string' || redisUrl.trim().length === 0)) {
      throw new Error('Missing required inventory-service Redis config: REDIS_URL');
    }
  }
  ...
  if (isEnabled(config, 'RABBITMQ_ENABLED')) {
    ...
    if (!isTest && (typeof rabbitmqUrl !== 'string' || rabbitmqUrl.trim().length === 0)) {
      throw new Error('Missing required inventory-service RabbitMQ config: RABBITMQ_URL');
    }
    if (!isTest && (typeof exchange !== 'string' || exchange.trim().length === 0)) {
      throw new Error('Missing required inventory-service RabbitMQ config: RABBITMQ_EXCHANGE');
    }
  }
