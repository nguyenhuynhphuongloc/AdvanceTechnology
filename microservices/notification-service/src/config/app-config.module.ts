...
export function validateEnvironment(config: Record<string, unknown>) {
  ...
  if (missingDbKeys.length > 0) {
    throw new Error(`Missing required notification-service config: ${missingDbKeys.join(', ')}`);
  }
  ...
  if (isEnabled(config, 'RABBITMQ_ENABLED')) {
    ...
    if (!isTest && (typeof rabbitmqUrl !== 'string' || rabbitmqUrl.trim().length === 0)) {
      throw new Error('Missing required notification-service RabbitMQ config: RABBITMQ_URL');
    }
    if (!isTest && (typeof exchange !== 'string' || exchange.trim().length === 0)) {
      throw new Error('Missing required notification-service RabbitMQ config: RABBITMQ_EXCHANGE');
    }
  }
